...
// 下位机软件版本号
const uint8_t SoftVersionString_QDB[] = "I:_STM;V1.0.0_20240703_TEST_YZL"; // QDB:驱动板
...
void EthernetInterfaceHandler(void)
{
    ...
    if (SysCtrl.Ethernet.Data[SysCtrl.Ethernet.ETH_ReceiveLength.data - 1] == 0x5E5D
            && SysCtrl.Ethernet.Data[SysCtrl.Ethernet.ETH_ReceiveLength.data - 2] == 0x5C5A)
    {
        dSum = Crs(SysCtrl.Ethernet.Data, SysCtrl.Ethernet.ETH_ReceiveLength.data - 4);
        rSum = SysCtrl.Ethernet.Data[SysCtrl.Ethernet.ETH_ReceiveLength.data - 4] +
               (SysCtrl.Ethernet.Data[SysCtrl.Ethernet.ETH_ReceiveLength.data - 3] << 16);
    }
...
void EthernetReceiveSuccessString_Updata(uint32_t ParameterLen, uint16_t *Send)
{
    uint16_t temp[40], num = 0, i;
    uint32_t SUM;
    volatile uint32_t PackageCnt = 0;
...
void ONLINEUPDATE1(void *parameter)
{
    ...
    volatile uint16_t *recvptr = NULL;
    recvptr =  parameter;
    updata_size.bytes.lbyte = *recvptr++;
    updata_size.bytes.hbyte = *recvptr++;
...
#endif
    Program_Updata_Area(Updata_Program_Address, (uint32_t)parameter,
                        1024);  //将接收缓存的数据写入app flash，size为字节,updata_program_add不需要动
